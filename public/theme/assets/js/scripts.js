!(function (NioApp, $) {
    ...
    NioApp.DataTable = function (elm, opt) {
        if ($(elm).exists()) {
            $(elm).each(function () {
                ...
                var btn = (has_export) ? '<"dt-export-buttons btn btn-sm btn-dim btn-danger d-flex align-center"<"dt-export-title d-none d-md-inline-block">B>' : '', btn_cls = (has_export) ? ' with-export' : '';
    ...
    // DataTable Init @v1.0
    let db = NioApp.DataTable.init = function () {
        ...
        NioApp.DataTable('.datatable-init-custom', {
            rowId: 'id',
            responsive: {
                details: true
            },
